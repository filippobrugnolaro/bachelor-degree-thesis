...
\noindent \intro{In questo capitolo vengono descritti i processi di verifica e validazione del prodotto, i \textit{tool} utilizzati e le metodologie applicate per valutare il corretto funzionamento e la qualità del prodotto.}\\
...
i \textit{test} effettuati sono riferiti alla soluzione del problema corrispondente a questi dati di \textit{input} resi coerenti.\\[0.2cm]
...
\end{table}
\newpage
\noindent La Tabella \ref{tab:test-max-it} riporta i risulati del \textit{test} e adotta la seguente notazione:
\begin{itemize}
...
\end{table}
\newpage
\noindent La Tabella \ref{tab:test-risoluzione} riporta i risulati del \textit{test} e adotta la seguente notazione:
\begin{itemize}
