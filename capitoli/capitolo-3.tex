...
\noindent \intro{In questo capitolo viene sposta l'analisi dei requisiti effettuata durante lo stage, nella quale vengono descritte le funzionalità tramite i casi d'uso.}\\
...
\req{R42-Q-D}{L'algoritmo utilizza differenti tecniche di ottimizzazione}{-}
\req{R43-Q-F}{L'algoritmo utilizza il multithreading per cercare più soluzioni ammissibili}{-}
\end{longtable}
\end{center}%
...
\centering \textbf{Requisito} & \centering \textbf{Descrizione} & \centering \textbf{Use Case} \arraybackslash \\
\hline 
\req{R44-P-O}{L'algoritmo di ottimizzazione deve restituire un risultato entro 10 minuti dal tempo di lancio dello stesso}{-}
\end{longtable}
\end{center}%
...
\centering \textbf{Requisito} & \centering \textbf{Descrizione} & \centering \textbf{Use Case} \arraybackslash \\
\hline  
\req{R45-V-O}{La \textit{form} deve essere eseguita sull'ambiente di esecuzione \textit{.NET Framework}}{-}
\req{R46-V-O}{La \textit{form} e l'algoritmo devono essere codificate in $C\#$}{-}
\req{R47-V-O}{La versione utilizzata di $C\#$ deve essere $7.3$}{-}
\req{R48-V-O}{La versione utilizzata di \textit{.NET Framework} deve essere $4.8$}{-}
\req{R49-V-O}{L'algoritmo finale deve fornire una soluzione ammissibile}{-}
\end{longtable}
\end{center}%