...
\textit{database} ed eseguire nuovamente il ciclo. Questo è necessario
...
\noindent L'utente, all'apertura della \textit{Windows form}, modifica i parametri di \textit{input} che sono:
\begin{itemize}
...
\begin{itemize}
    ...
    l'articolo più richiesto e ne viene comprato ogni
    record 
    \item \textbf{\textit{HighestArtRequestByForn}:} viene scelto
    l'articolo più richiesto, viene scelto il fornitore che
    offre il prezzo minore e vengono comprati tutti gli articoli
    da quel fornitore
    \item \textbf{\textit{HighestVariationByArt}:} viene scelto
    l'articolo più variazione rispetto alla stessa data dell'anno
    precendente e ne viene comprato ogni
    record 
    \item \textbf{\textit{HighestVariationByForn}:} viene scelto
    l'articolo più variazione rispetto alla stessa data dell'anno
    precendente, viene scelto il fornitore che
    offre il prezzo minore e vengono comprati tutti gli articoli
    da quel fornitore
\end{itemize}
...
In pratica per ogni \textit{range} di copertura di ogni articolo viene preso il record con il fornitore che offre il fabbisogno al prezzo più vantaggioso.\\
...
per ogni \textit{range} di copertura di ogni articolo sia necessario scegliere la miglior
combinazione tra data d'ordine e fornitore.\\
Data l'alta grandezza del vicinato non è dunque possibile visitarlo per intero,
...
il \textit{seed} all'interno del costruttore, viene utilizzato il
...
di \textit{Random} allo stesso valore e dunque veniva generato lo stesso
...
di minimi locali. Questo accade perché tramite le funzione
...
Se la mossa non viene trovata, questa viene aggiunta nella \textit{Tabu list} sia nel caso in cui venga effettuata che nel caso in cui non venga effettuata perchè non migliorante.\\
...
\end{itemize}
\newpage
\noindent La prima funzione controlla per ogni \textit{range} di
...
alla miglior combinazione per ogni \textit{range} di copertura di
...
\begin{itemize}
    \item creazione della \textit{Windows form}
...
organizzata un brevissima discussione con il \textit{tutor}
...
l’obiettivo di facilitare il \textit{debug}
...
elencare eventuali \textit{feature} ed estensioni che si possono
...
Si potrebbe valutare, in base alle risorse a disposizione, la creazione di un algoritmo genetico oppure di una combinazione tra la \textit{Tabu search} e un altro algoritmo
...
\begin{itemize}
    \item compatibilità con il sistema \textit{ERGDIS};
...
del \textit{framework} \textit{.NET}. Supporta astrazione, ereditarietà e polimorfismo
...
\begin{figure}[!h] 
 ...
 \end{figure}
\noindent \textit{Visual Studio 2019} è un ambiente di sviluppo integrato (IDE) fornito da \textit{Microsoft}.
...
\textit{software} \textit{Microsoft}, quali \textit{Windows API} e \textit{Windows form}.\\
\textit{Visual Studio 2019} supporta il \textit{refactoring} del codice e \textit{intellisense}, uno strumento per
l’autocompletamento del codice. Il \textit{debugger} integrato funziona sia a
...
\textit{Visual Studio}. In particolare crea estensioni che vengono utilizzate tramite \textit{Visual Studio} per velocizzare la scrittura di applicazioni. Gli strumenti da loro forniti sono
molteplici, in particolare hanno sviluppato controlli \textit{.NET} di interfaccia utente (\textit{UI}).
